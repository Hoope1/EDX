...
class ImageProcessor:
    ...
    def process_single_method(self, image_path: Path, method_name: str, 
                             method_func, output_base: Path) -> Tuple[str, bool, str]:
        """Verarbeite ein Bild mit einer Methode"""
        try:
            # Output-Pfad bestimmen
            if self.output_config['preserve_structure']:
                # Behalte Unterordner-Struktur
...
class BatchProcessor:
    ...
    def process_parallel(self, images: List[Path]):
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            # Starte alle Jobs
            future_to_image = {
                executor.submit(process_image_batch, args): args[0] 
...
Beispiele:
  ...
  %(prog)s -i dataset -o processed --skip-existing
    ...
    parser.add_argument('-i', '--input_dir', required=True, type=Path,
                       help='Eingabe-Verzeichnis mit Bildern')
    parser.add_argument('-o', '--output_dir', required=True, type=Path,
                       help='Ausgabe-Verzeichnis für Ergebnisse')
    
    # Optionale Argumente
    parser.add_argument('--methods', nargs='+', 
                       choices=[m[0] for m in METHODS],
                       help='Nur bestimmte Methoden verwenden')
    parser.add_argument('--sequential', action='store_true',
                       help='Sequentielle statt parallele Verarbeitung')
    parser.add_argument('--skip-existing', action='store_true',
                       help='Überspringe bereits verarbeitete Bilder')
    parser.add_argument('--workers', type=int, metavar='N',
                       help='Anzahl paralleler Prozesse (default: auto)')
    parser.add_argument('--gpu', choices=['auto', 'on', 'off'], default='auto',
                       help='GPU-Nutzung (default: auto)')
    parser.add_argument('--config', type=Path,
                       help='Alternative Config-Datei')
    parser.add_argument('--formats', nargs='+', metavar='EXT',
                       help='Zusätzliche Bildformate (z.B. .webp .jp2)')
    ...
    if args.formats:
        temp_config['supported_formats'] = [f if f.startswith('.') else f'.{f}' 
